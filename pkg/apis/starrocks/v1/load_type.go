...
type StarRocksLoadSpec struct {
	...
	// When HPA policy is enabled with a fixed replica count: every time the
	// starrockscluster CR is applied, the replica count of the StatefulSet
	// object in K8S will be reset to the value specified by the 'Replicas'
	// field, erasing the value previously set by HPA. So operator will set
	// it to nil when HPA policy is enabled.
