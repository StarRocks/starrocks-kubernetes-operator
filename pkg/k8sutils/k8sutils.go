...
func ClearFinalizersOnServices(ctx context.Context, k8sclient client.Client, namespace string, names []string) (bool, error) {
	...
	for _, name := range names {
		var svc corev1.Service
		if err := k8sclient.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, &svc); err != nil && !apierrors.IsNotFound(err) {
			return false, err
		}
		...
		if len(svc.Finalizers) != 0 {
			svc.Finalizers = nil
			cleared = false
			if err := UpdateClientObject(ctx, k8sclient, &svc); err != nil {
				return cleared, err
			}
		}
	}
...
func ClearFinalizersOnAutoscaler(ctx context.Context, k8sclient client.Client, namespace, name string, version srapi.AutoScalerVersion) (bool, error) {
	cleared := true
	var autoscaler client.Object
	switch version {
	case srapi.AutoScalerV1:
	...
	case srapi.AutoScalerV2Beta2:
		autoscaler = &v2beta2.HorizontalPodAutoscaler{}
	default:
		return false, errors.New(fmt.Sprintf("the autoscaler type %s is not supported.", version))
	}

	if err := k8sclient.Get(ctx, types.NamespacedName{Name: name, Namespace: namespace}, autoscaler); err != nil && !apierrors.IsNotFound(err) {
		return false, err
	}

	if len(autoscaler.GetFinalizers()) != 0 {
		autoscaler.SetFinalizers(nil)
		cleared = false
		if err := UpdateClientObject(ctx, k8sclient, autoscaler); err != nil {
			return cleared, err
		}
	}
...
func GetHorizontalPodAutoscaler(ctx context.Context, k8sclient client.Client, namespace, name string, version srapi.AutoScalerVersion) (client.Object, error) {
	var autoscaler client.Object
	switch version {
	case srapi.AutoScalerV1:
		autoscaler = &v1.HorizontalPodAutoscaler{}
	...
	case srapi.AutoScalerV2Beta2:
		autoscaler = &v2beta2.HorizontalPodAutoscaler{}
	default:
		return nil, errors.New(fmt.Sprintf("the autoscaler type %s is not supported.", version))
	}

	return autoscaler, k8sclient.Get(ctx, types.NamespacedName{Name: name, Namespace: namespace}, autoscaler)
}
