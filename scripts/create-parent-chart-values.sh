...
rm -f $HOME_PATH/helm-charts/charts/kube-celerdata/values.yaml
cat <<EOF >$HOME_PATH/helm-charts/charts/kube-celerdata/values.yaml
...
operator:
`awk '{printf("  %s\n", $0);}' $HOME_PATH/helm-charts/charts/kube-celerdata/charts/operator/values.yaml`

celerdata:
`awk '{printf("  %s\n", $0);}' $HOME_PATH/helm-charts/charts/kube-celerdata/charts/celerdata/values.yaml`
EOF
